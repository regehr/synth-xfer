...
- Ternary instructions (for example `fshl` and `fshr`) must not be evaluated with the same bitwidth settings as binary instructions. For ternary instructions, cap precision checks at bitwidth `5`. Soundness checks for ternary instructions should go all the way to 64 bits.
...
- Ternary-op override: for ternary instructions such as `fshl`/`fshr`, use `--exact-bw 5` for `eval-final`/`eval-intermediate` (keep other flags unchanged). For soundness checks using `verify`, you should go all the way to 64 bits for ternary operations.
